...
class JwtAuth
{
    ...
    {
        $this->key = 'Esta es la clave secreta zonnebloem'; // es una clave randon
    }
    ...
    public function singup($userpass, $password, $getToken = null)
    {
        ...
        $user = User::where([ // guarda en un objeto
            ...
            'userpass' => $userpass,
            'password' => $password
            // Coje la primera conicidencia con first()
        ])->first();

        // 2.- Comprobar si las credenciales son correctas.
        $singup = false;
        if (is_object($user)) { // Comprobamos si es un objecto
            $singup = true;
        }

        // 3.- Generar el token con los datos del usuario identificado.
        if ($singup) {
            // Cuando se decodifique el token todos estos datos seran vicible
            $token = array(
                'sub' => $user->id,
                'carnet' => $user->carnet,
            ...
            // la key es la clave del backend(esta arriba)
        ...
        // ojo.- Creamos nuestro servisProvider para utilizar el helpers
    }
